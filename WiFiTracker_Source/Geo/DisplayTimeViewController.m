...
- (void)viewDidLoad
{
    ...
    /*
     ...
     NSLog(@"Networks == %@",[stumbler networks]);
     NSLog(@"Desc == %@",[stumbler description]);
     */
    ...
    if (![[NSUserDefaults standardUserDefaults] valueForKey:@"IS_WIFI_NAME_ADDED"])
    {
        ...
        locationNameViewController *lvc=[[locationNameViewController alloc]initWithNibName:@"locationNameViewController" bundle:nil];
        
        UINavigationController *navView=[[UINavigationController alloc]initWithRootViewController:lvc];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if ([[NSUserDefaults standardUserDefaults] valueForKey:@"IS_WIFI_NAME_ADDED"])
    {
        ...
        //        [app CheckTheLatlog:NO]; // call the checking fuction
        
        [app fetchSSIDInfo:NO];
        //----------
...
#pragma mark - Get stength

-(void)getStrength:(WiFiManagerRef)newManager
{
    /*
     WiFiManagerRef manager = WiFiManagerClientCreate(kCFAllocatorDefault, 0);
     //    CFArrayRef devices = WiFiManagerClientCopyDevices(newManager);

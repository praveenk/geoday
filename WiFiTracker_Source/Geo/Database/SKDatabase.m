...
- (void)insertArray:(NSArray *)dbData forTable:(NSString *)table {
	...
	NSMutableString *sql = [NSMutableString stringWithCapacity:30];
...
- (void)MakeCSVFromDatabase:(NSString *)sql
{
			...
			for (int i = 0 ; i < sqlite3_column_count(statement) ; i++) {
				...
				if (sqlite3_column_decltype(statement,i) != NULL &&
					...
					if((char *)sqlite3_column_text(statement,i) != NULL){
                        ...
                        //                        NSLog(@"%@",[NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, i)]);
                        
                        
                        if (csv.length==0) {
                            csv=[NSString stringWithFormat:@"%@,%@\n", [NSString stringWithUTF8String:sqlite3_column_name(statement,i)], [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, i)]];
                        }
                        else{
                            NSString *str=[NSString stringWithFormat:@"%@,%@\n", [NSString stringWithUTF8String:sqlite3_column_name(statement,i)], [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement, i)]];
                            csv=[csv stringByAppendingString:str];
    ...
    [csv writeToFile:[documentsDirectory stringByAppendingPathComponent:@"location.csv"]

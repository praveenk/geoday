...
-(void)getiBeaconDeviceDetails :(NSArray *)arrDetails
{
    ...
    if (arrDetails.count==1)
    {
        lblDeviceUUID.text=(NSString *)[arrDetails objectAtIndex:0];
     
        lblDeviceDetails.text=@"Unknown";
        return;
    ...
    NSString *format = @"Major: %@ • Minor: %@ • %@ • Acc: %@ ";
    NSString *strValue = [NSString stringWithFormat:format, [arrDetails objectAtIndex:1], [arrDetails objectAtIndex:2], [arrDetails objectAtIndex:3], [arrDetails objectAtIndex:4]];
    lblDeviceDetails.text=strValue;
